...
(function($, window, undefined) {
    ...
    var rhinoSlider = function(element, opts) {
        var settings = $.extend({}, $.fn.rhinoslider.defaults, opts), $slider = $(element), effects = $.fn.rhinoslider.effects, preparations = $.fn.rhinoslider.preparations,
        //internal variables
        vars = {
            isPlaying : false,
    ...
    $.fn.rhinoslider.effects = {
        ...
        chewyBars : function($slider, params, callback) {
            var vars = $slider.data('slider:vars'), settings = params.settings, direction = params.direction, values = [], preSlide = function($slider, settings, $li) {
                var vars = $slider.data('slider:vars');
                $li.html('<div class="' + vars.prefix + 'partContainer">' + $li.html() + '</div>');
                var part = $li.html(), width = $slider.width(), height = $slider.height();
                for ( i = 1; i < settings.parts; i++) {
                    $li.html($li.html() + part);
                }
                var $parts = $li.children('.' + vars.prefix + 'partContainer'), partValues = [];
                switch(direction) {
                    case 'toLeft':
            ...
            slideBars = function($slider, settings) {
                ...
                $nextParts.each(function(i) {
                    var $this = $(this), newValues = [], aniMap = {
                        opacity : 1
                    };
